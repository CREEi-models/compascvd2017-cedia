...
keep if agegroup ==30 | agegroup==31
...
foreach var of varlist atlantique quebec ontario prairie bc {
	gen base_`var' = `var'[1]
	replace `var' = `var'/base_`var'
	drop base_`var'
}

keep if !mod(year,2)
outsheet using "params/trends/trend-pop30_male.csv", $opt
restore
...
keep if !mod(year,2)
outsheet using "params/trends/trend-pop30_female.csv", $opt
