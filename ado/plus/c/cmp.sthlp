...
{p 4 6 0}
* Each equation's linear functional (XB) can appear on the right side of any equation, even when it is modeled as latent (not fully observed), and
...
{help cnreg:cnreg}, {help intreg:intreg}, {help truncreg:truncreg}, {help heckman:heckman}, {help heckprob:heckprob}, {help heckoprobit:heckoprobit}, 
...
{cmp:cmd} uses adaptive quadrature by default to integrate likelihoods over the unobserved random effects (see {manhelp xtmixed R}). Quadrature estimates these 
...
{synopt :{cmdab:cond:ition(}{it:c d} [, {cmdab:eq:uation(#}{it:eqno}|{it:eqname}{cmd:)}]{cmd:)}}condition a {cmd:pr}, {cmd:e}, or {cmd:ystar} statistic on bounding another equation's outcome between {it:c} and {it:d}{p_end}
...
on the bounding of another equation's. E.g., one can use the option to predict the probability of a student's math score being between 5 and 10 if her reading
...
estimates for ordered probit equations can be conditioned too. The Heckman selection model examples below illustrate.
...
{pstd}
To compute marginal effects on the probability of a particular outcome, such as (1,1) from a bivariate probit model, you can preserve the data, set the outcome variables to the values of interest,
...
{phang}. {stata cmp (fem_inc=kids male_educ) (male_educ=kids other_inc fem_work), ind("cond(fem_inc,$cmp_cont,$cmp_left)" $cmp_cont)}{p_end}
...
{phang}. {stata preserve}{p_end}
{phang}. {stata replace insure = 2}{p_end}
{phang}. {stata margins, dydx(nonwhite) expression(exp(predict(lnl))) force} // marginal effect on probability of insure=2{p_end}
{phang}. {stata restore}{p_end}
...
{phang}. {stata margins, dydx(*) predict(eq(#1) pr outcome(#2)) force}{p_end}
